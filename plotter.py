...
class KBIPlotter:
	# add support for n-component systems; how to visualize???
	def __init__(self, model):
		""" visualizing kbi analysis """
		self.model = model
		
	def make_figures(self):
		# create plots --> x-axis will be solute mol.
			...
			for i, mol_1 in enumerate(self.model.unique_mols):
				for j, mol_2 in enumerate(self.model.unique_mols):
					if i <= j:
						...
						ax[ij_combo].set_title(f"{self.model.mol_name_dict[mol_1]}-{self.model.mol_name_dict[mol_2]}\n{self.model.df_comp[f'phi_{self.model.solute}'][s]:.2f} $\phi_{{{self.model.mol_name_dict[self.model.solute]}}}$")
						ij_combo += 1
	...
	def x_basis(self, basis):
		...
		if basis.lower() == "mol":
			...
			xplot = self.model.phis[:,self.model.solute_loc].flatten()
			x_lab = '\phi'
		return xplot, x_lab
	...
	def plot_composition_kbis(self, basis):
		...
		xplot, x_lab = self.x_basis(basis)
		...
		ax.set_xlabel(f'${x_lab}_{{{self.model.mol_name_dict[self.model.solute]}}}$')
	...
	def plot_dln_gammas(self, basis):
		...
		for i, mol in enumerate(self.model.unique_mols):
			ax.scatter(xplot, self.model.dlngamma_dxs[:,i], c=colors[i], linewidth=1.8, marker='s', label=self.model.mol_name_dict[mol])
	...
	def plot_ln_gammas(self, basis):
		...
		colors = plt.cm.jet(np.linspace(0,1,len(self.model.unique_mols)+1))
		for i, mol in enumerate(self.model.unique_mols):
			ax.scatter(xplot, np.log(self.model.gammas[:,i]), c=colors[i], linewidth=1.8, marker='s', label=self.model.mol_name_dict[mol])
		ax.legend(fontsize=11)
		ax.set_xlim(-0.05, 1.05)
		ax.set_xticks(ticks=np.arange(0,1.1,0.1))
		ax.set_xlabel(f'${x_lab}_{{{self.model.mol_name_dict[self.model.solute]}}}$')
	...
	def plot_Gex(self, basis):

		xplot, x_lab = self.x_basis(basis)
		xplot0 = self.xplot0(basis)

		fig, ax = plt.subplots(1, 1, figsize=(5, 4))
		ax.scatter(xplot0, self.model.Gex0, c='m', linewidth=1.8, marker='s')
		ax.set_ylim(-0.05, max(self.model.Gex0)+0.05)
		ax.set_xlim(-0.05, 1.05)
		ax.set_xticks(ticks=np.arange(0,1.1,0.1))
		ax.set_xlabel(f'${x_lab}_{{{self.model.mol_name_dict[self.model.solute]}}}$')
	...
	def plot_UNIQUAC_IP_fit(self):
		try:
			self.model.uniquac_du
		except:
			self.model.fit_UNIQUAC_IP()
		
		def UNIQUAC_GM(z, du):
			zc = 10
			l = (zc / 2) * (self.model.r - self.model.q) - (self.model.r - 1)
			tau = np.exp(-du / (self.model.Rc * self.model.T_sim))
			rbar = z @ self.model.r
			qbar = z @ self.model.q
			# Calculate phi and theta (segment and area fractions)
			phi = z * (1 / rbar[:, None]) * self.model.r
			theta = z * (1 / qbar[:, None]) * self.model.q
			# Solve for free energy of mixing GM 
			GM_comb = self.model.Rc * self.model.T_sim * (np.sum(z * np.log(phi), axis=1) + (zc / 2) * (z * (np.log(theta) - np.log(phi))) @ self.model.q)
			GM_res = -self.model.Rc * self.model.T_sim * (z * np.log(np.dot(theta , tau))) @ self.model.q
			return GM_comb + GM_res

		Gmix_fit0 = np.zeros(len(self.model.nrtl_Gmix0))
		Gmix_fit0[1:-1] = UNIQUAC_GM(self.model.z, self.model.uniquac_du)

		xplot0 = self.xplot0("mol")

		fig, ax = plt.subplots()
		ax.scatter(xplot0, self.model.uniquac_Gmix0, marker='o', linewidth=1.8, color='k', label='Simulated', zorder=10)
		ax.scatter(xplot0, Gmix_fit0, marker='s', linewidth=1.8, color='tab:red', label='UNIQUAC')			
		ax.set_xlim(-0.05,1.05)
		ax.set_xlabel(f'$x_{{{self.model.mol_name_dict[self.model.solute]}}}$')
		ax.set_ylabel(f'$\Delta G_{{mix}}$ $[kJ$ $mol^{{-1}}]$')
		ax.legend(fontsize=11, loc='upper center')
		ymin = np.round(min([min(self.model.uniquac_Gmix), min(Gmix_fit0)])-0.1, 1)
	...
	def plot_NRTL_IP_fit(self):
		...
		Gmix_fit0 = np.zeros(len(self.model.nrtl_Gmix0))
		Gmix_fit0[1:-1] = NRTL_GE(self.model.z, tau12, tau21)

		xplot0 = self.xplot0("mol")

		fig, ax = plt.subplots()
		ax.scatter(xplot0, self.model.nrtl_Gmix0, marker='o', linewidth=1.8, color='k', label='Simulated', zorder=10)
		ax.scatter(xplot0, Gmix_fit0, marker='s', linewidth=1.8, color='tab:red', label='NRTL')			
		ax.set_xlim(-0.05,1.05)
		ax.set_xlabel(f'$x_{{{self.model.mol_name_dict[self.model.solute]}}}$')
		ax.set_ylabel(f'$\Delta G_{{mix}}$ $[kJ$ $mol^{{-1}}]$')
		ax.legend(fontsize=11, loc='upper center')
		ymin = np.round(min([min(self.model.nrtl_Gmix), min(Gmix_fit0)])-0.1, 1)
		ax.set_ylim(ymin, 0.05)
	...
	def plot_FH_chi_fit(self):
		...
		fig, ax = plt.subplots()
		ax.scatter(pplot, Gmix0, color='k', marker='o', linewidth=1.8, label="Simulated")
		ax.scatter(pplot, fh_gmix0, color='tab:red', marker='s', linewidth=1.8, label='FH')
		ax.legend(fontsize=11, loc='upper center')
		ymin = np.round(min([min(self.model.fh_Gmix), min(self.model.G_mix_xv)])-0.1, 1)
		ax.set_ylim(ymin, 0.05)
		ax.set_xlabel(f'$\phi_{{{self.model.mol_name_dict[self.model.solute]}}}$')
		ax.set_xlim(-0.05, 1.05)
...
class PhaseDiagramPlotter:
	...
	def __init__(self, model: ThermoModel):
		...
		self.unique_mols = self.model.mol_id
		self.num_components = len(self.unique_mols)


	def make_figures(self):
		# based on num components make figures
	...
	def binary_phase_diagram_I0_heatmap_widomline(self, ymin: float = 0.01, ymax: float = 1., basis: str = 'vol', num_contours=100, show_fig: bool = False):
		...
		# add widom line
		ax.plot(self.widom(basis)[t, self.model.solute_loc], self.model.I0_max['T'], c='k', linestyle='solid', lw=2, zorder=len(self.model.T_values)+1)
...
#### ternary plots ####
	...
	def plot_ternary_z(self, T, num_contours: int = 20, colormap: str = 'jet', show_fig: bool = False):
		...
		if self.model.save_dir is not None:
			plt.savefig(f'{self.model.save_dir}{self.model.model_name}_GM_at_{T}K.png')
		if show_fig:
			plt.show()
		else:
			plt.close()

	def plot_ternary_spbi(self, T, arr_name: str, num_contours: int = 20, colormap: str = 'jet', show_fig: bool = False):
		''' mixing free energy at T '''
		...
		T_ind = np.abs(self.model.T_values - T).argmin()
		T_plot = self.model.T_values[T_ind]
		arr_dict = {"d2GM": self.model.d2GM, "GM": self.model.GM}
		arr_T = arr_dict[arr_name][T_ind]
		a, b, c = self.model.z[:,0], self.model.z[:,1], self.model.z[:,2]

		valid_mask = (a >= 0) & (b >= 0) & (c >= 0) & ~np.isnan(arr_T) & ~np.isinf(arr_T)
		a = a[valid_mask]
		b = b[valid_mask]
		c = c[valid_mask]
		values = arr_T[valid_mask]
		...
		tp = ax.tricontourf(a, b, c, values, cmap=colormap, alpha=1, aspect=25, edgecolors='none', levels=num_contours)
		fig.colorbar(tp, ax=ax, aspect=25, label='$\Delta G_{mix}$ [kJ mol$^{-1}$]')

		sp_arr = self.model.x_sp[T_ind]
	...
	def plot_ternary_Io(self, T, num_contours: int = 20, colormap: str = 'jet', show_fig: bool = False):
		''' mixing free energy at T '''
		...
		if self.model.save_dir is not None:
			plt.savefig(f'{self.model.save_dir}{self.model.model_name}_Io_at_{T}K.png')
		if show_fig:
			plt.show()
		else:
			plt.close()
	...
	def ternary_binodals_fTemp(self, colormap: str = 'jet', show_fig: bool = False):
		...
		if self.model.save_dir is not None:
			plt.savefig(f'{self.model.save_dir}{self.model.model_name}_binodals_temperature_dependence_molfrac.png')
		if show_fig:
			plt.show()
		else:
			plt.close()
...
# 		""" for visualing thermodynamic property predictions """